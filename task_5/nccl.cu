 ...
 * условиями реализуется с использованием NCCL
 */
...
#define ncclT ncclFloat
#else
#define T double
#define MAX std::fmax
#define STOD std::stod
#define ncclT ncclDouble
#endif
...
void transfer_data(const int rank, const int size, T *A_host, T *A_device, ncclComm_t comm, uint32_t h, uint32_t w, cudaStream_t stream = 0)
{
    ncclGroupStart();
    ...
    if (rank != 0)
    {
        ...
        ncclSend(A_host + w, w, ncclT, rank - 1, rank - 1, comm, stream);
        ncclRecv(A_host, w, ncclT, rank - 1, rank, comm, stream);
    ...
    if (rank != size - 1)
    {
        ...
        ncclSend(A_host + w * (h - 2), w, ncclT, rank + 1, rank + 1, comm, stream);
        ncclRecv(A_host + (w * (h - 1)), w, ncclT, rank + 1, rank, comm, stream);
...
int main(int argc, char *argv[])
{
    ...
    // Создание коммуникатора для nccl
    ncclUniqueId nccl_id;
    if (rank == 0) 
        ncclGetUniqueId(&nccl_id);
    MPI_Bcast(&nccl_id, sizeof(nccl_id), MPI_BYTE, 0, MPI_COMM_WORLD);
    ncclComm_t comm;
    ncclCommInitRank(&comm, size, nccl_id, rank);
    ...
    *error = accuracy + 1;
    ...
    for (iter = 0; iter < iter_max && *error > accuracy; iter += skipped_checks_counts)
    {
        for (uint32_t k = 0; k < skipped_checks_counts; k += 2)
        {
            ...
            transfer_data(rank, size, A, A_dev, net_height, net_size, comm, stream);
            ...
            transfer_data(rank, size, A, Anew_dev, net_height, net_size, comm, stream);
        }
